...
export default function App() {
  ...
  return (
    <div className="wrap">
      ...
      <div className="hdr">
        <div>
          <div className="logo">삼성육부 · 종합 관제판</div>
          <div className="sub-text">OpenClaw 삼성육부 대시보드</div>
        </div>
        <div className="hdr-r">
          <span className={`chip ${syncOk ? 'ok' : syncOk === false ? 'err' : ''}`}>
            {syncOk ? '✅ 동기화 정상' : syncOk === false ? '❌ 서버 미실행' : '⏳ 연결 중…'}
          </span>
          <span className="chip">{activeEdicts.length}개 지시</span>

...
export default function MonitorPanel() {
  ...
  const handleWakeAll = async () => {
    ...
    if (!toWake.length) { toast('모든 Agent가 이미 온라인입니다'); return; }
    toast(`${toWake.length}개 Agent를 깨우는 중...`);
    for (const a of toWake) {
      try { await api.agentWake(a.id); } catch { /* ignore */ }
    }
    toast(`${toWake.length}개 깨우기 명령을 보냈습니다. 30초 후 상태를 새로고침합니다`);
    setTimeout(() => loadAgentsStatus(), 30000);
  };
    ...
    <div>
      ...
      {asData && asData.ok && (
        <div className="as-panel">
          <div className="as-header">
            ...
            <span className={`as-gw ${gwCls}`}>Gateway: {koText(gw?.status || '未知')}</span>
          ...
          <div className="as-grid">
            {filtered.map((a) => {
              ...
              return (
                <div key={a.id} className="as-card" title={`${koRole(a.role)} · ${koText(a.statusLabel)}`}>
                  <div className={`as-dot ${a.status}`} />
                  <div style={{ fontSize: 22 }}>{a.emoji}</div>
                  <div style={{ fontSize: 12, fontWeight: 700 }}>{koDept(a.label)}</div>
                  <div style={{ fontSize: 10, color: 'var(--muted)' }}>{koRole(a.role)}</div>
                  <div style={{ fontSize: 10, color: 'var(--muted)' }}>{koText(a.statusLabel)}</div>
                  {a.lastActive ? (
          ...
          <div className="as-summary">
            ...
            <span style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--muted)' }}>
              확인 시각 {(asData.checkedAt || '').substring(11, 19)}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="duty-grid">
        {DEPTS.map((d) => {
          ...
          const statusText = isBlocked ? '⚠️ 차단됨' : isActive ? '⚙️ 실행 중' : hb.status === 'active' ? '🟢 활성' : '⚪ 대기';
            ...
            <div key={d.id} className={`duty-card ${cardCls}`}>
              <div className="dc-hdr">
                ...
                <div className="dc-info">
                  <div className="dc-name">{koDept(d.label)}</div>
                  <div className="dc-role">{koRole(d.role)} · {koRole(d.rank)}</div>
                </div>
                ...
                {myTasks.length > 0 ? (
                  myTasks.map((t) => (
                    <div key={t.id} className="dc-task" onClick={() => setModalTaskId(t.id)}>
                      ...
                      <div className="dc-task-title">{koText(t.title || '(제목 없음)')}</div>
                      {t.now && t.now !== '-' && (
                        <div className="dc-task-now">{koText(t.now.substring(0, 70))}</div>
                      )}
                      <div className="dc-task-meta">
                        <span className={`tag st-${t.state}`}>{stateLabel(t)}</span>
                        {t.block && t.block !== '无' && (
                          <span className="tag" style={{ borderColor: '#ff527044', color: 'var(--danger)' }}>🚫{koText(t.block)}</span>
                        )}
                      </div>
                    </div>
                  ))

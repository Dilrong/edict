...
const CAT_META: Record<string, { icon: string; color: string; desc: string }> = {
  '政治': { icon: '🏛️', color: '#6a9eff', desc: '글로벌 정치 동향' },
  '军事': { icon: '⚔️', color: '#ff5270', desc: '군사 및 분쟁' },
  '经济': { icon: '💹', color: '#2ecc8a', desc: '경제와 시장' },
  'AI大模型': { icon: '🤖', color: '#a07aff', desc: 'AI와 대형 모델 동향' },
};
...
function catLabel(cat: string): string {
  return CAT_LABELS[cat] || koText(cat);
}
...
export default function MorningPanel() {
  ...
  const [refreshLabel, setRefreshLabel] = useState('⟳ 지금 수집');
  ...
  const refreshNews = async () => {
    ...
    try {
      ...
      toast('수집을 시작했습니다. 업데이트를 자동 확인 중입니다…', 'ok');
      let count = 0;
      if (pollRef.current) clearInterval(pollRef.current);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 4 }}>🌅 오늘의 브리핑</div>
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>
            {dateStr && `${dateStr} | `}
            {morningBrief?.generated_at && `${morningBrief.generated_at} 수집 | `}
            총 {totalNews}개 주요 뉴스
          </div>
        </div>
      ...
      {!Object.keys(cats).length ? (
        <div className="mb-empty">데이터가 없습니다. 오른쪽 위의 "지금 수집"으로 오늘 브리핑을 가져오세요</div>
      ) : (
            ...
            return (
              <div className="mb-cat" key={cat}>
                <div className="mb-cat-hdr">
                  ...
                  <span className="mb-cat-name" style={{ color: meta.color }}>{catLabel(cat)}</span>
                  <span className="mb-cat-cnt">{scored.length}개</span>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mb-info">
                            ...
                            <div className="mb-summary">{koText(item.summary || item.desc || '')}</div>
...
function SubConfigPanel({
  ...
  return (
    <div className="sub-config" style={{ marginBottom: 20, padding: 16, background: 'var(--panel2)', borderRadius: 12, border: '1px solid var(--line)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {allCats.map((cat) => {
            ...
            return (
              ...
              >
                <span>{meta.icon}</span>
                <span style={{ fontSize: 12 }}>{catLabel(cat)}</span>
      ...
      {/* Custom Feeds */}
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>사용자 정보 소스</div>
        {(config.custom_feeds || []).map((f, i) => (
          <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4, fontSize: 11 }}>
            ...
            <span style={{ color: 'var(--acc)' }}>{catLabel(f.category)}</span>

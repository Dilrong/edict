...
// Curated community skill sources
const COMMUNITY_SOURCES = [
  {
    label: 'obra/superpowers',
    emoji: '⚡',
    stars: '66.9k',
    desc: '완성형 개발 워크플로 스킬 모음',
    skills: [
...
export default function SkillsConfig() {
  ...
  const submitAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (r.ok) {
        toast(`✅ 스킬 ${formData.name}을(를) ${koDept(addForm.agentLabel)}에 추가했습니다`, 'ok');
        setAddForm(null);
        loadAgentConfig();
      } else {
  ...
  const submitAddRemote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (r.ok) {
        toast(`✅ 원격 스킬 ${skillName}을(를) ${agentId}에 추가했습니다`, 'ok');
  ...
  const handleUpdate = async (skill: RemoteSkillItem) => {
    ...
    try {
      ...
      if (r.ok) {
        toast(`✅ 스킬 ${skill.skillName}을(를) 업데이트했습니다`, 'ok');
        loadRemoteSkills();
      } else {
  ...
  const handleRemove = async (skill: RemoteSkillItem) => {
    ...
    try {
      ...
      if (r.ok) {
        toast(`🗑️ 스킬 ${skill.skillName}을(를) 삭제했습니다`, 'ok');
        loadRemoteSkills();
        loadAgentConfig();
      } else {
        toast(r.error || '삭제 실패', 'err');
      }
    } catch {
      toast('서버 연결 실패', 'err');
    }
  ...
  if (!agentConfig?.agents) {
    return <div className="empty">불러올 수 없습니다</div>;
  }
        ...
        {agentConfig.agents.map((ag) => (
          <div className="sk-card" key={ag.id}>
            <div className="sk-hdr">
              ...
              <span className="sk-name">{koDept(ag.label)}</span>
              <span className="sk-cnt">{(ag.skills || []).length}개 스킬</span>
            </div>
            <div className="sk-list">
              {!(ag.skills || []).length ? (
                <div className="sk-empty">스킬 없음</div>
              ) : (
                (ag.skills || []).map((sk) => (
                  <div className="sk-item" key={sk.name} onClick={() => openSkill(ag.id, sk.name)}>
                    <span className="si-name">📦 {sk.name}</span>
                    <span className="si-desc">{koText(sk.description || '설명 없음')}</span>
    ...
    <div>
          ...
          <div style={{ marginTop: 14, background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              ...
              >
                ...
                {agentConfig.agents.map((ag) => (
                  <option key={ag.id} value={ag.id}>{ag.emoji} {koDept(ag.label)} ({ag.id})</option>
                ))}
              </select>
            </div>
      ...
      {remoteLoading ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px', background: 'var(--panel)', borderRadius: 12, border: '1px dashed var(--line)' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--muted)' }}>원격 스킬 없음</div>
          <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6 }}>커뮤니티 소스에서 빠르게 가져오거나 URL을 직접 추가하세요</div>
        </div>
      ) : (
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                    ...
                    <span style={{
                      ...
                      {sk.status === 'valid' ? '✓ 유효' : '✗ 파일 없음'}
                    </span>
                    <span style={{ fontSize: 11, color: 'var(--muted)', background: 'var(--panel2)', padding: '2px 8px', borderRadius: 6 }}>
                      {agInfo?.emoji} {koDept(agInfo?.label) || sk.agentId}
                    </span>
                  </div>
                  {sk.description && (
                    <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 4 }}>{koText(sk.description)}</div>
                  )}
      ...
      {addForm && (
        <div className="modal-bg open" onClick={() => setAddForm(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div style={{ fontSize: 11, color: 'var(--acc)', fontWeight: 700, letterSpacing: '.04em', marginBottom: 4 }}>
                {koDept(addForm.agentLabel)}에 스킬 추가
              </div>
              <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 18 }}>＋ 새 Skill</div>
              ...
              >
                <b style={{ color: 'var(--text)' }}>📋 Skill 작성 규칙</b>
                <br />
                • 스킬 이름은 <b style={{ color: 'var(--text)' }}>소문자 영어 + 하이픈</b>을 사용하세요
                <br />
                • 생성 후 템플릿 파일 `SKILL.md`가 만들어집니다
                <br />
                • Agent가 관련 작업을 받으면 스킬이 <b style={{ color: 'var(--text)' }}>자동 활성화</b>됩니다
              </div>
          ...
          <div className="modal" style={{ maxWidth: 520 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <form onSubmit={submitAddRemote} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, display: 'block', marginBottom: 6 }}>소스 URL <span style={{ color: '#ff5270' }}>*</span></label>
                  <input
                    type="url"
                    required

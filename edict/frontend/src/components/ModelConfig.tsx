...
import { api } from '../api';
import { koDept, koRole } from '../i18n';
...
export default function ModelConfig() {
  ...
  const applyModel = async (agentId: string) => {
    ...
    try {
      ...
      if (r.ok) {
        setStatusMap((p) => ({ ...p, [agentId]: { cls: 'ok', text: '✅ 제출됨, Gateway 재시작 중(약 5초)' } }));
      ...
      setStatusMap((p) => ({ ...p, [agentId]: { cls: 'err', text: '❌ 서버에 연결할 수 없음' } }));
    }
  };
  ...
  return (
    <div>
            ...
            <div className="mc-card" key={ag.id}>
              <div className="mc-top">
                ...
                <div>
                  ...
                  <div className="mc-role">{koRole(ag.role)}</div>
                </div>
              </div>
      ...
      <div style={{ marginTop: 24, marginBottom: 8 }}>
        <div className="sec-title">배정 채널</div>
          ...
          <button className="btn btn-p" disabled={channelSel === (agentConfig?.dispatchChannel || 'feishu')}
            onClick={async () => {
              try {
                ...
                if (r.ok) { setChannelStatus('✅ 저장됨'); toast('배정 채널을 변경했습니다', 'ok'); loadAgentConfig(); }
                else setChannelStatus('❌ ' + (r.error || '실패'));
              } catch { setChannelStatus('❌ 연결할 수 없음'); }
        ...
        <div style={{ fontSize: 11, color: 'var(--muted)' }}>자동 배정에 사용할 OpenClaw 알림 채널입니다. `openclaw.json`에 해당 channel이 설정되어 있어야 합니다.</div>
      </div>

      {/* Change Log */}
      <div style={{ marginTop: 24 }}>
        <div className="sec-title">변경 로그</div>
        <div className="cl-list">
          {!changeLog?.length ? (
            <div style={{ fontSize: 12, color: 'var(--muted)', padding: '8px 0' }}>변경 내역 없음</div>
          ) : (
            [...changeLog]
              .reverse()
              .slice(0, 15)

    ...
    <div>
      ...
      {alive.length > 0 && (
        <div className="off-activity">
          <span>🟢 현재 활성:</span>
          {alive.map((o) => (
            <span key={o.id} style={{ fontSize: 12 }}>{o.emoji} {koRole(o.role)}</span>
          ))}
      ...
      <div className="off-kpi">
        ...
        <div className="kpi">
          <div className="kpi-v" style={{ fontSize: 16, paddingTop: 4 }}>{koRole(officialsData.top_official) || '—'}</div>
          <div className="kpi-l">최고 공적</div>
        </div>
      </div>
      ...
      <div className="off-layout">
          ...
          {offs.map((o) => {
            ...
            return (
              ...
              >
                ...
                <span>{o.emoji}</span>
                <span style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, fontWeight: 700 }}>{koRole(o.role)}</div>
                  <div style={{ fontSize: 10, color: 'var(--muted)' }}>{koDept(o.label)}</div>
                </span>
        ...
        <div className="off-detail">
          {sel ? (
            ...
            <div className="empty">왼쪽 관원을 선택해 상세 정보를 확인하세요</div>
          )}
        </div>
      </div>
    </div>
...
function OfficialDetail({
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>
            {koDept(o.label)} · <span style={{ color: 'var(--acc)' }}>{o.model_short || o.model}</span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
            🏅 {koRole(o.rank)} · 공적 점수 {o.merit_score}
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div className={`hb ${hb.status}`} style={{ marginBottom: 4 }}>{koText(hb.label)}</div>
          {o.last_active && <div style={{ fontSize: 10, color: 'var(--muted)' }}>활성 {o.last_active}</div>}
          <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 2 }}>
            {o.sessions}개 세션 · {o.messages}개 메시지
          </div>
        </div>
      </div>
      ...
      {/* Participated Edicts */}
      <div>
        <div className="sec-title">참여 지시({edicts.length}개)</div>
        {edicts.length === 0 ? (
          <div style={{ fontSize: 12, color: 'var(--muted)', padding: '8px 0' }}>지시 기록 없음</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {edicts.map((e) => (
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: 12 }}>{koText(e.title.substring(0, 35))}</span>

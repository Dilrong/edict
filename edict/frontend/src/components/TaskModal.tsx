...
export default function TaskModal() {
  ...
  const doReview = async (action: string) => {
    ...
    try {
      ...
      if (r.ok) {
        toast(`✅ ${task.id} ${labels[action]} 완료`, 'ok');
        loadAll();
        close();
      } else {
        toast(r.error || '작업 실패', 'err');
  ...
  const doSchedAction = async (action: string) => {
    if (action === 'scan') {
      try {
        ...
        if (r.ok) toast(`🔍 스캔 완료: ${r.count || 0}개 조치`, 'ok');
    ...
    const labels: Record<string, string> = { retry: '재시도', escalate: '상향 조정', rollback: '롤백' };
    const reason = prompt(`${labels[action]} 사유를 입력하세요(비워둘 수 있음):`);
  ...
  const handleStop = () => {
    const reason = prompt('중지 사유를 입력하세요(비워둘 수 있음):');
    if (reason === null) return;
    doTaskAction('stop', reason);
  };

  const handleCancel = () => {
    if (!confirm(`${task.id} 작업을 취소할까요?`)) return;
    const reason = prompt('취소 사유를 입력하세요(비워둘 수 있음):');
    if (reason === null) return;
    ...
    <div className="modal-bg open" onClick={close}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="modal-title">{koText(task.title || '(제목 없음)')}</div>
          ...
          {activeStage && (
            <div className="cur-stage">
              ...
              <div className="cs-info">
                <div className="cs-dept" style={{ color: deptColor(activeStage.dept) }}>{koDept(activeStage.dept)}</div>
                <div className="cs-action">현재 단계: {activeStage.action}</div>
              </div>
              <span className={`hb ${hb.status} cs-hb`}>{koText(hb.label)}</span>
            </div>
          )}
          ...
          <div className="task-actions">
            {canStop && (
              <>
                <button className="btn-action btn-stop" onClick={handleStop}>⏸ 작업 중지</button>
                <button className="btn-action btn-cancel" onClick={handleCancel}>🚫 작업 취소</button>
              </>
            )}
            {canResume && (
              <button className="btn-action btn-resume" onClick={() => doTaskAction('resume', '실행 재개')}>▶️ 실행 재개</button>
            )}
          ...
          <div className="sched-section">
            <div className="sched-head">
              <span className="sched-title">🧭 태자 스케줄링</span>
              <span className="sched-status">
                {sched ? `${sched.enabled === false ? '비활성' : '실행 중'} · 임계값 ${sched.stallThresholdSec || 180}s` : '불러오는 중...'}
              </span>
            </div>
            <div className="sched-grid">
              ...
              <div className="sched-kpi"><div className="k">상향 단계</div><div className="v">{!sched?.escalationLevel ? '없음' : sched.escalationLevel === 1 ? '문하성' : '상서성'}</div></div>
              <div className="sched-kpi"><div className="k">배정 상태</div><div className="v">{sched?.lastDispatchStatus || 'idle'}</div></div>
            </div>
            {sched && (
              <div className="sched-line">
                {sched.lastProgressAt && <span>최근 진행 {formatDashboardDateTime(sched.lastProgressAt)}</span>}
                {sched.lastDispatchAt && <span>최근 배정 {formatDashboardDateTime(sched.lastDispatchAt)}</span>}
                <span>자동 롤백 {sched.autoRollback === false ? '꺼짐' : '켜짐'}</span>
                {sched.lastDispatchAgent && <span>대상 {sched.lastDispatchAgent}</span>}
              </div>
            )}
            <div className="sched-actions">
              <button className="sched-btn" onClick={() => doSchedAction('retry')}>🔁 배정 재시도</button>
              ...
              <button className="sched-btn" onClick={() => doSchedAction('scan')}>🔍 지금 스캔</button>
            </div>
          </div>
            ...
            <div className="m-rows">
              <div className="m-row">
                ...
                <div className="mr-val">
                  ...
                  {(task.review_round || 0) > 0 && <span style={{ fontSize: 11, color: 'var(--muted)', marginLeft: 8 }}>총 {task.review_round}차 협의</span>}
                </div>
              </div>
              <div className="m-row">
                <div className="mr-label">실행 부서</div>
                <div className="mr-val"><span className={`tag dt-${(task.org || '').replace(/\s/g, '')}`}>{koDept(task.org) || '—'}</span></div>
              </div>
              {task.eta && task.eta !== '-' && (
              ...
              {task.block && task.block !== '无' && task.block !== '-' && (
                <div className="m-row"><div className="mr-label" style={{ color: 'var(--danger)' }}>차단 항목</div><div className="mr-val" style={{ color: 'var(--danger)' }}>{koText(task.block)}</div></div>
              )}
              {task.now && task.now !== '-' && (
                <div className="m-row" style={{ gridColumn: '1/-1' }}>
                  <div className="mr-label">현재 진행</div>
                  <div className="mr-val" style={{ fontWeight: 400, fontSize: 12 }}>{koText(task.now)}</div>
                </div>
              )}
              {task.ac && (
                <div className="m-row" style={{ gridColumn: '1/-1' }}>
                  <div className="mr-label">검수 기준</div>
                  <div className="mr-val" style={{ fontWeight: 400, fontSize: 12 }}>{koText(task.ac)}</div>
                </div>
              )}
            </div>
          </div>
          ...
          {flowLog.length > 0 && (
            <div className="m-section">
              <div className="m-sec-label">흐름 로그({flowLog.length}개)</div>
                    ...
                    <div className="fl-item" key={i}>
                      ...
                      <div className="fl-content">
                        <div className="fl-who">
                          <span className="from" style={{ color: col }}>{koDept(fl.from)}</span>
                          <span style={{ color: 'var(--muted)' }}> → </span>
                          <span className="to" style={{ color: deptColor(fl.to || '') }}>{koDept(fl.to)}</span>
                        </div>
                        <div className="fl-rem">{koText(fl.remark)}</div>
                      </div>
                    </div>
                  );
                })}
    ...
    <div className="todo-section">
      ...
      <div className="todo-list">
        {todos.map((td) => {
          ...
          const stLabel = td.status === 'completed' ? '완료됨' : td.status === 'in-progress' ? '진행 중' : '시작 대기';
          ...
          return (
            <div className={`todo-item ${itemCls}`} key={td.id}>
              <div className="t-row">
                ...
                <span className="t-title">{koText(td.title)}</span>
                <span className={`t-status ${stCls}`}>{stLabel}</span>
              </div>
              {td.detail && <div className="todo-detail">{koText(td.detail)}</div>}
            </div>
          );
        })}
      </div>
...
function LiveActivitySection({
  ...
  if (data.lastActive) agentParts.push(`마지막 활동: ${formatDashboardDateTime(data.lastActive)}`);
  ...
  return (
    <div className="la-section">
      <div className="la-header">
        ...
        <span className="la-agent">{agentParts.join(' · ') || '불러오는 중...'}</span>
      </div>
      ...
      {phaseDurations.length > 0 && (
        <div style={{ padding: '4px 0 8px', borderBottom: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 600 }}>⏱ 단계별 소요 시간</span>
            {data.totalDuration && <span style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--muted)' }}>총 소요 {koText(data.totalDuration)}</span>}
          </div>
          {phaseDurations.map((p, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, margin: '2px 0', fontSize: 11 }}>
                <span style={{ minWidth: 48, color: 'var(--muted)', textAlign: 'right' }}>{koDept(p.phase)}</span>
      ...
      {ts && (
        <div style={{ padding: '4px 0 8px', borderBottom: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <span style={{ fontSize: 11, fontWeight: 600 }}>📊 실행 진행률</span>
      ...
      {rs && (rs.totalTokens || rs.totalCost) && (
        <div style={{ padding: '4px 0 8px', borderBottom: '1px solid var(--line)', display: 'flex', gap: 12, alignItems: 'center' }}>
          <span style={{ fontSize: 11, fontWeight: 600 }}>📈 리소스 사용량</span>
          ...
          <div className="la-flow-wrap">
            {flowItems.map((a, i) => (
              <div className="la-entry la-tool" key={`flow-${i}`}>
                ...
                <span className="la-body"><b>{koDept(a.from)}</b> → <b>{koDept(a.to)}</b>　{koText(a.remark || '')}</span>
        ...
        {grouped.size > 0 ? (
          ...
          !flowItems.length && (
            <div className="la-empty">
              {koText(data.message || data.error || 'Agent가 아직 진행 상황을 보고하지 않았습니다(progress 명령 대기 중)')}
            </div>
          )
        )}
      </div>
  ...
  if (a.kind === 'progress') {
    return (
      <div className="la-entry la-assistant">
        <span className="la-icon">🔄</span>
        <span className="la-body">{agBadge}<b>현재 진행: </b>{koText(a.text)}</span>
  ...
  if (a.kind === 'todos') {
    ...
    return (
      <div className="la-entry" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 2 }}>
        <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 2 }}>{agBadge}📝 실행 계획</div>
        {items.map((td) => {
          ...
          return (
            <div key={td.id} style={style}>
              ...
              {d && d.type === 'added' && <span style={{ color: '#3b82f6', fontSize: 9, marginLeft: 4 }}>🆕신규</span>}
            </div>
          );
        })}
        {a.diff?.removed?.map((r) => (
          <div key={r.id} style={{ opacity: 0.4, textDecoration: 'line-through' }}>🗑 {koText(r.title)}</div>
        ))}
      </div>
    );
  }
      ...
      <>
        {a.thinking && (
          <div className="la-entry la-thinking">
            <span className="la-icon">💭</span>
            <span className="la-body">{agBadge}{koText(a.thinking)}</span>
            <span className="la-time">{time}</span>
          </div>
        )}
        {a.tools?.map((tc, i) => (
        ...
        {a.text && (
          <div className="la-entry la-assistant">
            <span className="la-icon">🤖</span>
            <span className="la-body">{agBadge}{koText(a.text)}</span>
            <span className="la-time">{time}</span>
          </div>
        )}
      </>
  ...
  if (a.kind === 'tool_result') {
    ...
    return (
      <div className={`la-entry la-tool-result ${ok ? 'ok' : 'err'}`}>
        ...
        <span className="la-body">{agBadge}<span className="la-tool-name">{a.tool || ''}</span>{a.output ? koText(a.output.substring(0, 150)) : ''}</span>
        <span className="la-time">{time}</span>
      </div>
    );
  }
